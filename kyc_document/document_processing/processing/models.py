...
class ModelLoader:
    ...
    def __call__(self, json_file: Path, device='gpu'):
        ...
        elif self.json_file['Type'] == 'Classifier':
    ...
    def __load_classifier(self):
        ...
        postprocessings = []
        for outp in self.json_file['Output']:
            classes = outp.get('Classes', [])
            postprocessings.append(
                MultiClassPostprocessing(
                    classes,
                    verbose=self.verbose
...
class YoloDetectorModel(Model):
    ...
    def predict(self, img: Union[Path, np.ndarray]):
        ...
        # Исправляем формат для belarus модели: если [21, 8400], транспонируем в [8400, 21]
        # Postprocessing ожидает [num_boxes, num_classes+4], где num_classes+4 должно быть вторым измерением
        if len(bboxes.shape) == 2:
            n_classes = len(self.postprocessing.labels)
            expected_cols = n_classes + 4  # 4 координаты + n_classes
            # Если количество столбцов не совпадает с ожидаемым, транспонируем
            if bboxes.shape[1] != expected_cols and bboxes.shape[0] == expected_cols:
