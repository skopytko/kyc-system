...
class ModelInference:
    ...
    def __predict_onnx(self, tensor:  np.ndarray):
        ...
        # Проверяем формат входных данных модели
        input_shape = self.model.get_inputs()[0].shape
        # Если модель ожидает NCHW формат [N, C, H, W], а тензор в формате NHWC [N, H, W, C]
        if len(input_shape) == 4 and len(tensor.shape) == 4:
            # Проверяем, нужно ли транспонировать: если второй размер модели = 3 (каналы), а у тензора последний = 3
            if input_shape[1] == 3 and tensor.shape[-1] == 3:
                # Транспонируем из NHWC в NCHW
