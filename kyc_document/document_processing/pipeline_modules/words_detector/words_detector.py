...
class WordsDetector(BaseModule):
    ...
    def predict(self, img: Union[str, Path, np.ndarray]) -> dict:
        ...
        if loaded is None or not isinstance(loaded, np.ndarray) or loaded.ndim < 2 or loaded.shape[0] < 1 or loaded.shape[1] < 1:
            return {self.model_name: {"bbox": []}}
        img = loaded
    ...
    def predict_transform(self, img: Union[str, Path, np.ndarray]) -> dict:
        ...
        if img is None or not isinstance(img, np.ndarray) or img.ndim < 2 or img.shape[0] < 1 or img.shape[1] < 1:
            return {
                self.model_name: {
                    "bbox": [],
                    "warped_img": [],
