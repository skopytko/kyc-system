...
class QualityChecker(object):
    ...
    def check_image_quality(self, image):
        ...
        """
        self.perform_image(image)
        # Map predicted blur labels to numeric scores where:
        #  0.0 -> sharp (best quality)
        #  0.5 -> medium blur
        #  1.0 -> heavy blur (worst quality)
        label_to_score = {
            'Blur0': 0.0,
            'Blur05': 0.5,
            'Blur5': 0.5,
        ...
        # If nothing matched known blur labels, avoid division by zero.
        # Be conservative: treat as bad quality.
        if not result_list:
            return 0.0

        max_level_for_normalization = len(result_list)
        quality_level = 1 - (sum(result_list) / max_level_for_normalization)
        return float(quality_level)